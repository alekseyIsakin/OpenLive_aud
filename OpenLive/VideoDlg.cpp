...
void CVideoDlg::AdjustButtonsNormal(int cx, int cy)
{
	...
	if (m_cbxLang.GetSafeHwnd() != NULL)
		m_cbxLang.MoveWindow(cx / 2 - 370, cy - 50, 120, 42, TRUE);;
...
{
	LPARAM i_sel = m_cbxLang.GetCurSel();

	OnBnClickedBtnclose();
	GetParent()->SendMessage(WM_JOINCHANNEL, 0, i_sel);
}
...
void CVideoDlg::UpdateLangCBox(Tokens token, int curSel)
{
	int nIndex = 0;
	m_cbxLang.ResetContent();

	for (auto langIt = token.GetLngBgnIterator(); langIt != token.GetLngEndIterator(); langIt++) {
		CString str((*langIt).langShort.c_str());

		m_cbxLang.InsertString(nIndex, str);
		nIndex++;
	}
	m_cbxLang.SetCurSel(curSel);
}
...
void CVideoDlg::InitCtrls()
{
	...
	m_cbxLang.Create(WS_VISIBLE | WS_CHILD | CBS_AUTOHSCROLL | CBS_DROPDOWNLIST, CRect(0, 0, 1, 1), this, IDC_CBXROLE_VIDEO);
	m_cbxLang.SetFont(&m_ftDes);
	...
	m_cbxLang.MoveWindow(rcClient.Width() - 80, rcClient.Height() - 84, 300, 48, TRUE);
...
void CVideoDlg::OnShowWindow(BOOL bShow, UINT nStatus)
{
	...
	if (bShow && GetSafeHwnd() != NULL) {
		...
		if (nRole == 1)
			m_cbxLang.SetCurSel(0);
		else
			m_cbxLang.SetCurSel(1);
	}
}
